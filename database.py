...
class ChessDatabase:
    ...
    def create_tables(self):
        self.cursor.execute("CREATE TABLE IF NOT EXISTS repertoires (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, color TEXT CHECK(color IN ('White', 'Black')) NOT NULL)")
        self.cursor.execute("CREATE TABLE IF NOT EXISTS positions (id INTEGER PRIMARY KEY AUTOINCREMENT, fen TEXT UNIQUE NOT NULL)")
        self.cursor.execute("CREATE TABLE IF NOT EXISTS moves (id INTEGER PRIMARY KEY AUTOINCREMENT, repertoire_id INTEGER, from_position_id INTEGER, to_position_id INTEGER, uci TEXT NOT NULL, comment TEXT, FOREIGN KEY(repertoire_id) REFERENCES repertoires(id), FOREIGN KEY(from_position_id) REFERENCES positions(id), FOREIGN KEY(to_position_id) REFERENCES positions(id))")
        self.conn.commit()
    ...
    def delete_move(self, move_id):
        ...
        self.cursor.execute("SELECT id FROM moves WHERE repertoire_id = ? AND from_position_id = ?", (rep_id, to_pos_id))
    ...
    def get_move_by_id(self, move_id):
        self.cursor.execute("SELECT p.fen FROM moves m JOIN positions p ON m.to_position_id = p.id WHERE m.id = ?", (move_id,))
        row = self.cursor.fetchone()
    ...
    def add_move(self, repertoire_id, from_fen, to_fen, uci, comment=""):
        ...
        self.cursor.execute("SELECT id FROM moves WHERE repertoire_id=? AND from_position_id=? AND uci=?", (repertoire_id, from_id, uci))
        ...
        if existing:
            ...
            self.cursor.execute("INSERT INTO moves (repertoire_id, from_position_id, to_position_id, uci, comment) VALUES (?, ?, ?, ?, ?)", (repertoire_id, from_id, to_id, uci, comment))
            self.conn.commit()
            return self.cursor.lastrowid
    ...
    def get_moves_from_fen(self, repertoire_id, fen):
        ...
        if not row:
            return []
        from_id = row['id']
        self.cursor.execute("SELECT m.id, m.uci, m.comment, p.fen as to_fen FROM moves m JOIN positions p ON m.to_position_id = p.id WHERE m.repertoire_id = ? AND m.from_position_id = ?", (repertoire_id, from_id))
        return self.cursor.fetchall()
